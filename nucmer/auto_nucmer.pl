...
foreach my $subref (`ls $ref.cut/`){
	chomp $subref;
	my $refbasename = basename ($subref);

	foreach my $subquery (`ls $query.cut/`){
		chomp $subquery;
		my $querybasename = basename ($subquery);

		print NUCMER "/ifshk5/PC_PA_US/USER/shichch/bin/MUMmer3.23/nucmer -p $outpath/$refbasename.$querybasename $ref.cut/$subref $query.cut/$subquery\n";
	}
}
close NUCMER;
print "Done!\n";
...
print WORK "cat *.delta | grep -v $ref.cut | grep -v NUCMER >> all.delta.lst\n";
...
print WORK "perl $Bin/fastaDeal.pl -attr id:len $ref > $ref.len\n";
print WORK "perl $Bin/fastaDeal.pl -attr id:len $query > $query.len\n";
print WORK "perl $Bin/process_coord_v2.pl all.delta.filter.coords $ref.len > all.delta.filter.coords.ref.align.len\n";
print WORK "perl $Bin/process_coord_Query.pl all.delta.filter.coords $query.len > all.delta.filter.coords.query.align.len\n";
close WORK;

system ("qsub -l vf=5G -cwd $qub_parse work.sh ");
print "Done!\n";
