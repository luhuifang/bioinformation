...
use Data::Dumper;
use PerlIO::gzip;
#use lib "/ifs2/BC_GAG/Bin/Evolution/Tools/svg_kit/svg_package/Svg/";
use lib "/home/liuwq/Program_collect/Pair_end_figure/svg_kit";
use SVG;
use FontSize;

my $Bin="/ifs5/PC_PA_US/USER/fangy/bin/Old_bin/";
...
if($ARGV[0] eq "PRE"||$ARGV[0] eq "ALL")
{
   ...
   print STDERR "get contig position was finished.";
   &get_pair_info($soap_re_list,%scaff_list);
   print STDERR "get pair mapping information was finished.";
}

if($ARGV[0] eq "DRAW"||$ARGV[0] eq "ALL")
{
   &get_the_map();
   print STDERR "get the map was finished.";
}
...
sub get_the_map()
{
   ...
   while(<PAIR>)
   {
      ...
      else
      {
		 ...
		 elsif($distance>=24000&&$distance<=60000)
		 	{$hash_pair_info{$name}{40000}{$tmp[0]}=$tmp[2];}
      }
   }
   close PAIR;
   ...
   foreach my $name (keys(%contig_compose))
   {
      ...
      my $svg=SVG->new(width=>($length*10)/($bp_per_pix*9),height=>500);
      my $path=$svg->group(
      ...
      ###########################################画出pair-end关系支持
	  foreach my $key_2 (sort {$a<=>$b} keys (%{$hash_pair_info{$name}{40000}}))
	  {
		if ($key_2>=$start&&$hash_pair_info{$name}{40000}{$key_2}<=$end)
		{
			my $len=($hash_pair_info{$name}{40000}{$key_2}-$key_2)/$bp_per_pix;
      ...
      { 
         if ($key_2>=$start&&$hash_pair_info{$name}{20000}{$key_2}<=$end)
        {
      ...
      {  
         if ($key_2>=$start&&$hash_pair_info{$name}{10000}{$key_2}<=$end)
        {
      ...
      {  
        if ($key_2>=$start&&$hash_pair_info{$name}{5000}{$key_2}<=$end)
        {      
      ...
      {
      	 if ($key_2>=$start&&$hash_pair_info{$name}{2000}{$key_2}<=$end)
         {
      ...
      {  
         if ($key_2>=$start&&$hash_pair_info{$name}{700}{$key_2}<=$end)
         {
      ...
      {  
         if ($key_2>=$start&&$hash_pair_info{$name}{300}{$key_2}<=$end)
         {
...
sub get_contig_pos() 
{
   my $flag_1=0;my $flag_2=0;my $flag_3=0;my $stat=0;my $pos=0;my $end=0;
   my ($fa,%pos)=@_;
